        ...
        with InputAccordion(False, label="Detail Daemon", elem_id=self.elem_id('detail-daemon')) as gr_enabled:
            with gr.Row():
                with gr.Column(scale=2):                    
                    gr_amount_slider = gr.Slider(minimum=-1.00, maximum=1.00, step=.01, value=1.00, label="Detail Amount")
                    gr_start = gr.Slider(minimum=0.0, maximum=1.0, step=.01, value=0.5, label="Start")
                    gr_end = gr.Slider(minimum=0.0, maximum=1.0, step=.01, value=0.5, label="End") 
                    gr_bias = gr.Slider(minimum=0.0, maximum=1.0, step=.01, value=0.5, label="Bias")                                                                                                                          
                with gr.Column(scale=1, min_width=275):  
                    preview = self.visualize(False, 0.5, 0.5, 0.5, 0.1, 1, 0, 0, 0, True)                                 
            ...
            with gr.Accordion("More Knobs:", elem_classes=['detail-daemon-more-accordion'], open=False):
                with gr.Row():
                    with gr.Column(scale=2):   
                        ...
                        with gr.Row():
                            gr_amount = gr.Number(value=1.00, precision=4, step=.01, label="Amount", min_width=60)  
                    ...
                    with gr.Column(scale=1, min_width=275): 
                        ...
                        gr_smooth = gr.Checkbox(label="Smooth", value=False, min_width=60, elem_classes=['detail-daemon-smooth'])
