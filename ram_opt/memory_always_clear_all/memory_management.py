...
def free_memory(memory_required, device, keep_loaded=[], free_all=True):
    ...
    if free_all:
        memory_required = float('inf')  # Unlimited memory requirement
    ...
    offload_everything = ALWAYS_VRAM_OFFLOAD
    ...
    print("[RAM] Clearing unused objects and freeing RAM...")
    gc.collect()  # Force garbage collection
    torch.cuda.empty_cache()  # Clear GPU cache
    mem = psutil.virtual_memory()
    print(f"Available system RAM: {mem.available / (1024 * 1024):.2f} MB")
